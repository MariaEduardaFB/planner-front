...
const ViagensList = () => {
  ...
  const handleSearchParticipant = async () => {
    ...
    try {
      const response = await api.get(
        `/user-viagem/search?email=${participantEmail}`
      );
  ...
  const handleAddParticipant = async () => {
    if (!currentViagem.id) {
      setInviteError(
        "A viagem precisa ser selecionada antes de convidar participantes."
      );
    ...
    try {
      ...
      setInviteError(
        error.response?.data?.message || "Erro ao convidar participante."
      );
      setInviteSuccess(false);
    }
  };
  ...
  const handleRemoveParticipant = async (participantId) => {
    ...
    try {
      ...
      await api.delete(
        `/viagens/${currentViagem.id}/participantes/${participantId}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
  ...
  const [showParticipantDetailsModal, setShowParticipantDetailsModal] =
    useState(false);
    ...
    try {
      ...
      setCurrentViagem((prev) => ({
        ...
        atividades: prev.atividades.map((atividade) =>
          atividade.id === selectedActivity.id
  ...
  const handleDeleteActivity = async (atividadeId) => {
    ...
    try {
      ...
      setCurrentViagem((prev) => ({
        ...prev,
        atividades: prev.atividades.filter(
  ...
  const [showActivityDetailsModal, setShowActivityDetailsModal] =
    useState(false); // Modal de detalhes da atividade
  ...
  return (
    <Box className={classes.root}>
      ...
      >
        <Box className={classes.modalBox}>
          <Typography variant="h6" className={classes.modalTitle}>
            Criar Atividade para a Viagem: {currentViagem.pais} -{" "}
            {currentViagem.cidade}
          </Typography>
          ...
          <TextField
            ...
            onChange={(e) =>
              setNewActivity({ ...newActivity, descricao: e.target.value })
            }
            className={classes.modalTextField}
          />
          ...
          <TextField
            ...
            onChange={(e) =>
              setNewActivity({ ...newActivity, data: e.target.value })
            }
            className={classes.modalTextField}
            InputLabelProps={{
              shrink: true,
          ...
          <TextField
            ...
            onChange={(e) =>
              setNewActivity({ ...newActivity, horario: e.target.value })
            }
            className={classes.modalTextField}
            InputLabelProps={{
              shrink: true,
              ...
              >
                <Box>
                  ...
                  <Typography
                    variant="body2"
                    sx={{
                      color: p.UserViagem.confirmada ? "#4caf50" : "#f44336",
                    }}
                  >
      ...
      >
        ...
        >
          <Typography variant="h6" mb={2}>
            Adicionar Participante à Viagem: {currentViagem.pais} -{" "}
            {currentViagem.cidade}
          </Typography>
          <TextField
      ...
      <Modal open={showDetailsModal} onClose={() => setShowDetailsModal(false)}>
        ...
        >
          ...
          <Typography>
            ...
            {currentViagem.organizador?.name || "Não informado"}
          </Typography>
          <Typography>
            <strong>País:</strong> {currentViagem.pais}
          </Typography>
          <Typography>
            <strong>Estado:</strong> {currentViagem.estado}
          </Typography>
          <Typography>
            <strong>Cidade:</strong> {currentViagem.cidade}
          </Typography>
          <Typography>
            <strong>Data de Início:</strong> {currentViagem.dataInicio}
          </Typography>
          <Typography>
            <strong>Data Final:</strong> {currentViagem.dataFinal || "-"}
          </Typography>
          <Typography>
            <strong>Confirmada:</strong>{" "}
            {currentViagem.confirmada ? "Sim" : "Não"}
          </Typography>
          <Typography variant="h6" className={classes.modalTitle}>
            Participantes
          </Typography>
      ...
      >
        ...
        >
          ...
          {selectedActivity && (
            <>
              <Typography>
                <strong>Nome:</strong> {selectedActivity.titulo}
              </Typography>
              <Typography>
                <strong>Data:</strong> {selectedActivity.dataAtividade}
              </Typography>
            </>
          )}
        ...
        >
          ...
          {selectedParticipant && (
            <>
              ...
              <Typography>
                <strong>E-mail:</strong> {selectedParticipant.email}
              </Typography>
              <Typography>
                <strong>Status:</strong>{" "}
                <span
                  style={{
                    color: selectedParticipant.UserViagem.confirmada
                      ? "#4caf50"
                      : "#f44336",
                  }}
                >
                  {selectedParticipant.UserViagem.confirmada
                    ? "Confirmado"
                    : "Não Confirmado"}
                </span>
              </Typography>
